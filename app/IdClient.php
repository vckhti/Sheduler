...
class IdClient
{
    ...
    public static function handleNewIdentifications(?string $date_start = null, ?string $date_end = null)
    {
        ...
        foreach ($identifications['data'] as $row) {
            self::uploadIdentificationFiles($row, ['documentFile', 'documentFileAddress', 'signatureFile']);
        }
    ...
    {
        $instance_id = strtolower(trim(Config::get('app.instance_id')));
